...
$q=$_SESSION['loginid'];
$q2="SELECT b.book_id, r.name, b.reg_no,v.vehicle_name, b.date,b.time,b.mileage FROM `tbl_booking` b, `tbl_vehicle` v, `tbl_reg` r WHERE b.login_id=r.login_id AND v.vehicle_id=b.vehicle_id AND b.status=0 AND b.adviser=3;"; 
$result=mysqli_query($con,$q2) or die( mysqli_error($con));
if(mysqli_num_rows($result)==0)
{
?>
<div class="container" style="padding-top: 300px; padding-left: 400px;">
<h1 style="font-weight: bolder;">NO PENDING ASSIGNMENTS </h1>
  </div>
  <?php
}
else
{
?>  
<form method="POST" action="assignadv.php">
                        ...
                        <tbody>
                           ...
                           while ($row = mysqli_fetch_array($result))
                         {
                          ...
                          <tr style="font-size: 30px; font-weight: bold;">
                            ...
                            <td >
                                <select type="text" class="form-control" name="adname" id="adname" style="color: white;" required>
                            <?php 
                            $q3="SELECT * FROM `tbl_log` l, `tbl_adv` a WHERE l.login_id=a.login_id AND usertype='adviser'"; 
                            $r=mysqli_query($con,$q3) or die( mysqli_error($con));
                                while($r2=mysqli_fetch_array($r))
                                  {
                            ?>
                                <option value="<?php echo $r2['adv_id']?>" ><?php echo $r2['advname']?></option>
                            <?php 
                               }
                           ?>
                         </select>
                            ...
                            <input type="submit" class="btn btn-primary btn-icon-text" name="assign" value="assign"></td>
                          </tr>
                          <?php
                            }}
                          ?>
